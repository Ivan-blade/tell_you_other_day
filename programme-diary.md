...
+ 我去，始料未及，突然出现了很多bug
    + 首先是跨域问题，原生app不存在跨域的概念，所以需要将跨域注释，请求接口修改为绝对路径，这个解决了
    + 然后是样式和css问题，不和谐感挺严重的，主要是mavon-editor，还有组件交界问题
    + 当编辑内容之后，返回日记栏，appbar会消失？？？它没了你敢信？？？emmm。。。这个再说
    + 最奇怪的bug，吃完饭回来发现登录功能没法用了（后端是正常的）。。。这算什么事嘛。。。
    + 当然，祸不单行，出问题的不只有前端，后端也有一点点问题，主要是只要远程一关，系统就会终止进程，腾讯云好像没有这种问题，不过当时是windows，啧麻烦
    + ...我不太懂它怎么又能登录了。。。
    + 说不定我该转玄学专业了。。。

    + 多模块项目打包为jar包教程https://blog.csdn.net/baidu_41885330/article/details/81875395
    + 目前后端本地依赖改为了8.0.18，日后重新打包时别忘了改依赖为8.0.19（父模块先clean，在package）
    + 前端打包时别忘了跨域注释掉
    + nohup java -jar Ivan-spring-web-1.0-SNAPSHOT.jar &(后台挂起服务 这次进程pid = 9049，别忘了kill，变成僵尸进程还是有点麻烦的)
    + 它又不能登录了
    + 不清楚原因，看起来和并发有关，前端的数据的确返回给了后端，后端的确能看见日志，但是前端有时候接收不到数据。。。会不会是因为一个请求没有响应，其他的请求就不会处理了？迷惑行为

#### 2020-3-21
+ 今天把app扔到服务器上吧，顺便开启一下ftp服务
+ https://blog.csdn.net/dingpeiqiang/article/details/87829518
+ 出现了一点问题，共享目录下无法显示文件
+ 行吧，我感觉是服务器的问题，好像过于频繁的端口访问会触发安全机制还是什么的，会禁止访问。。。等等，这不是ddos吗？？？话说，这种程度的访问服务器就没了？？？？？？服了
+ 几乎就是服务器的问题了，响应速度很慢
+ 我去。。。这是什么。。。我配置了ftp服务之后，21端口的服务只要处理失败，同时9090端口的服务也会被阻塞掉。。。。？？？？这是什么惊世骇俗的操作？？？现在的服务器是这样的吗？？？等等，springboot内置了tomcat服务器，应该是和tomcat有关系？？？可能不是。ftp服务怎么会影响tomcat服务呢？这就离谱，虽然ftp连接了，但是仍然不能显示出该文件夹内的文件，周日，周一，周二。。。时间不多了，今天必须处理完ftp问题和前端页面的优化
+ ????????学长说我的产品没人用??????虽然的确造用户体验上存在一些问题，但是我不觉得这一点拿不出手吧?????我还能改呢?????用户体验这种事，改着改着不就好了吗
+ 问题找到了，appbar是因为在dialog收起的时候滑动了，所以收起来了。。。我怎么没想到的
+ 凶手找到了。。。是home页面引入组件中的的样式将home撑开了，所以出现了滚动条。。。但是具体问题在哪呢？
+ 等等。。。好像只要dialog里面有内容，页面就会被撑开？？？这是什么鬼
+ 害。。。只好取消appbar隐藏的功能了。。。但是内容撑开是为什么？再挣扎一下吧
+ dialog组件感觉怪怪的，home页面通过html body {overflow: hidden}全局样式处理了，但是另一边存在不同的问题，虽然不那么容易察觉
+ 对呈现逻辑做出了修改，菜单栏语言切换为中文
+ 哦豁。。。打包上限了。。。bug只能在手机上才能看见。。。模拟器好像也不行。。。如果换账号的话会有一些麻烦，还是准备一下面试吧。。。看看我简历上都写了啥
